...
export default function FlowPlayground(props) {
	...
	const updateVariableInputNode = () => {
		edges.forEach((eds) => {
			const sourceNode = nodes.filter((nds) => nds.id === eds.source)[0];
			const targetNode = nodes.filter((nds) => nds.id === eds.target)[0];
	...
	const disableSourceHandle = () => {
		if (edges?.length > 0) {
			edges.forEach((eds) => {
				...
				setNodes((nds) => {
					let availableNode = nds?.filter((nd) => nd?.id === sourceNode?.id)[0];
	...
	return (
		<div className="react-flow-container" ref={reactFlowWrapper}>
			<BotflowNav
				...
				exit={goBackToTable}
			/>
			<BotFlowMenu selectedNode={selectedNode} goBackMenu={goBackMenu} />
			<ReactFlow
				snapToGrid
				fitView
				...
				onNodeClick={(e) => {
					...
					e.currentTarget.onkeydown = function (x) {
						if (
							x.key === "Delete" &&
							nodes.filter((nds) => nds.id === id)[0].type !== "Receive"
						) {
							deleteNodeById(id);
						}
					};
